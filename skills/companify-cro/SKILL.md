---
name: companify-cro
description: Define o sistema de geração de receita. Use para estruturar company/revenue.md com monetização, pricing comercial, vendas e retenção econômica.
---

# Definir o sistema de receita (CRO)

## Protocolo operacional

- **Plano e progresso:** planejar o funil lead → oportunidade → venda →
  ativação → retenção → expansão antes de escrever o artefato final.
- **Fontes de verdade:** ler `company/business-model.md`,
  `company/marketing.md` e, quando já produzido, `company/finance.md` para o
  CAC máximo sustentável.
- **Escopo e idempotência:** pricing lidera com CFO, CPO e CMO consultados —
  não decidir preço isoladamente.
- **Validação:** cada métrica de receita usada (MRR, ARR, ARPA, ACV, win
  rate, sales cycle, pipeline coverage, NRR, GRR, churn, expansion revenue) só
  aparece quando aplicável ao modelo de negócio real.
- **Resumo final:** informar o pipeline, os canais comerciais, o pricing
  proposto e as premissas usadas.

## Fluxo

1. Descrever o funil completo: lead → oportunidade → venda → ativação →
   retenção → expansão, com as taxas de conversão conhecidas ou modeladas
   como cenário.
2. Definir monetização e pricing comercial, consultando `company/finance.md`
   para o CAC máximo sustentável e `company/product.md` para o que cada
   plano ou pacote efetivamente entrega.
3. Estruturar a estratégia de vendas, canais comerciais e parceiros.
4. Definir estratégia de expansão, upsell e cross-sell, e a leitura de churn
   e retenção econômica.
5. Definir métricas aplicáveis e o forecast de receita, sempre com premissas
   explícitas.
6. Salvar em `company/revenue.md`.

## Raciocínio do especialista

A pergunta central é: isso pode ser transformado em receita, de forma
repetível? Uma venda pontual não é um sistema de receita. Antes de recomendar
aumento de investimento comercial, cruzar CAC do canal com o CAC máximo
sustentável do CFO e com a retenção do canal reportada pelo CPO — otimizar
apenas a taxa de fechamento sem olhar para retenção produz crescimento que
não se sustenta.
