...
- **Fontes de verdade:** ler todos os artefatos de `company/`,
  `.companify/assumptions.md`, `.companify/decisions.md` e
  `.companify/metrics.md`.
- **Escopo e idempotência:** não concatenar os documentos — construir uma
  narrativa única. Preservar versões anteriores do plano; cada revisão soma
  uma entrada de versão, não substitui a história.
- **Validação:** nenhuma hipótese crítica aparece como fato; toda projeção
  cita premissa; todo risco crítico tem mitigação.
...
5. Listar premissas críticas, experimentos recomendados e decisões
   pendentes, puxando diretamente de `.companify/assumptions.md` e
   `.companify/decisions.md`.
6. Atualizar `company/README.md` com o status de cada artefato.
7. Registrar versão, data, mudanças relevantes, novas evidências, premissas
   invalidadas e decisões alteradas no topo de `company/business-plan.md`.
