...
- **Escopo e idempotência:** nunca substituir uma resposta já confirmada por
  uma inferência. Uma pergunta respondida não é reaberta sem nova evidência.
- **Validação:** cada resposta é classificada como fato, evidência,
...
1. Ler o Company Context atual e identificar quais dimensões têm campo em
   ...
   operação, tecnologia, equipe, recursos, objetivos, restrições, riscos,
   evidências.
...
4. Classificar cada resposta em [`FATO`, `EVIDÊNCIA`, `DECLARAÇÃO DO
...
não estava prevista. Preferir perguntas que testam uma decisão concreta
